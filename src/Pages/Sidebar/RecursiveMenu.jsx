                ...
                {subMenus.map((subItem, index) => (
                    <li key={index} className="relative cursor-pointer">
                        <NavLink
                            ...
                            className={({ isActive }) =>
                                ((isActive && subItem.subMenu.length == 0) ? `font-bold text-[#1890ff] ` : " ") + `${subMenuStyle} ` + 'flex gap-3 items-center'}
                            onClick={(e) => {
                                e.stopPropagation();
                                handleClick(subItem);
                            }}
                        >
                            {(subItem.subMenu.length > 0) &&
                                <span className={((openMenus.includes(subItem.name)) ? 'transition-all duration-200 ease-in-out rotate-90 text' : 'transition-all duration-200 ease-in-out rotate-0') + `${!navToggle && 'hidden'}`}><TbCaretRightFilled /></span>}
        ...
        <ul id="side-menu" className={`w-full float-none flex flex-col ${!navToggle ? 'items-center' : ''}`}>
            {menus.map((item, index) => (
                <li key={index} className='relative cursor-pointer mb-1 ' onClick={() => handleClick(item)}>
                    ...
                    >
                        <attr title={item?.name} className='text-lg'>{item.icon || <MdOutlineDashboard />}</attr>
