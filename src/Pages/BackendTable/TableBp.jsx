            ...
            <div className="w-full text-zinc-600 border text-sm p-3 mb-2 rounded-xl flex md:flex-row flex-col gap-y-2 md:justify-between md:items-center tableStyle bg-[#F6F7FB]">
...

                {
                    ...
                    <div className='w-full md:w-[50%] flex justify-end'>
                        ...
                        {exportStatus &&
                            <>
                                <div className='flex gap-5 md:justify-start justify-between border-l-2 border-l-zinc-300 pl-4'>
                                    {
                                        props?.exportLoader == true ? <div className='italic text-sm text-center'>Exporting...</div>
                                            :
                                            <div className="flex gap-2 items-center">
                                                <button className={actionButtonStyle + ' bg-violet-500 text-white hover:bg-violet-600'} onClick={() => props.exportFun('csv')}>
                                                    <CSVLink className='' data={props?.allData ?? []}>CSV</CSVLink>
                                                </button>
                                                <button className={actionButtonStyle + ' bg-blue-500 text-white hover:bg-blue-600'} onClick={() => props.exportFun('excel')}>Excel</button>
                                                <button className={actionButtonStyle + ' bg-teal-500 text-white hover:bg-teal-600'} onClick={() => props.exportFun('pdf')}>PDF</button>
                                            </div>
                                    }
                                </div>
                            </>}
                ...
                {Array.isArray(props?.dataList) && props?.dataList?.length > 0 &&
                    <>
                        <div className='px-4 pt-5 pb-3 w-full flex md:flex-row flex-col md:items-center flex-wrap gap-y-2 justify-between'>
                            ...
                            <Pagination
                                totalPages={parseInt(props?.lastPage) ?? 1}
                                page={props?.currentPage}
                                prev={prevPageFun}
                                next={nextPageFun}
                                jump={(p) => props?.gotoPage(p)} />

                            <div className='text-sm text-zinc-500 flex items-center gap-1'>
                                Items per page <select className="border-2 px-2 py-1.5 rounded-xl bg-slate-50 text-zinc-600 focus:outline-none focus:border-slate-400" value={pageSize} onChange={(e) => handleChange('pageSize', e.target?.value)}>
